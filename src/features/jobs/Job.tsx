  ...
  return <section className="page job-page" aria-labelledby="jobs-title">
    ...
    {preview.state !== "new_print_confirmation_required" ? <div className="job-columns">
      ...
      <div className="settle-panel">
        ...
        {outcome === "estimated" ? <label className="field">{copy("settlement.progress")}<div className="range-line"><input aria-label={copy("settlement.progress")} type="range" min="1" max="99" value={percent} onChange={(event) => setPercent(event.target.value)} /><strong className="data">{copy("settlement.percentValue", { percent })}</strong></div><small>{copy("settlement.estimatedUsage")}</small></label> : null}
